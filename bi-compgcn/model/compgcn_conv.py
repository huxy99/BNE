...
class CompGCNConv(MessagePassing):
	def __init__(self, in_channels, out_channels, num_rels, act=lambda x:x, params=None):
		...
		self.lin_loop	= binary.BinLinear(self.in_channels, self.out_channels, binary_weights=self.p.bin_lin)
		self.lin_in	= binary.BinLinear(self.in_channels, self.out_channels, binary_weights=self.p.bin_lin)
		self.lin_out	= binary.BinLinear(self.in_channels, self.out_channels, binary_weights=self.p.bin_lin)

		if self.p.bin_rescale:
			self.rescale_lin = binary.LearnedRescaleLayer0d((1, out_channels))
		else:
	...
	def message(self, x_j, edge_type, rel_embed, edge_norm, mode):
		weight 	= getattr(self, 'lin_{}'.format(mode))
		rel_emb = torch.index_select(rel_embed, 0, edge_type)
		xj_rel  = self.rel_transform(x_j, rel_emb)
		#xj_rel = self.bn(xj_rel)
		xj_rel = self.quantizer(xj_rel)
		out = weight(xj_rel)
