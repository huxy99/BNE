...
class Runner(object):
	...
	def __init__(self, params): #完成log建立、设备选择、加载数据、模型和优化器
		...
		self.load_data()
		self.model        = BNE_ConvE(self.edge_index, self.edge_type, params=self.p).to(self.device)
	...
	def predict(self, split='valid', mode='tail_batch'):
		...
		with torch.no_grad():
			...
			for step, batch in loop:
				...
				pred			= torch.sigmoid(self.model.forward(sub, rel))
	...
	def run_teacher(self):
		...
		self.teacher_args.activ = False
		self.teacher_args.bin_conv = 'real'
		self.teacher_args.bin_lin = 'real'
		self.teacher_args.bin_conv_rescale = False
		self.teacher_args.bin_lin_rescale = False
		self.teacher_args.bin_rescale = False


		self.logger.info(vars(self.teacher_args))

		self.teacher_model = BNE_ConvE(self.edge_index, self.edge_type, params=self.teacher_args).to(self.device)
		...
		for epoch in range(self.p.epoch):
			...
			if val_results['mrr'] > self.best_val_mrr:
				...
				if flag == 0 and kill_cnt == 25: 
					flag = 1
					self.logger.info("Model has early converged!!")
...
if __name__ == '__main__':
	...
	parser.add_argument('-dataset',		dest='dataset',         default='WN18RR',            help='Dataset to use, default: FB15k-237')
	...
	parser.add_argument('-activ', dest='activ', action='store_true', help='激活函数是否二值化')	 	
	...
	parser.add_argument('-bin_conv', dest='bin_conv', default='bi', choices=['bi','1-norm','real'], help='卷积是否二值化') 	
	parser.add_argument('-bin_lin', dest='bin_lin', default='bi', choices=['bi','1-norm','real'], help='fc是否二值化')
	parser.add_argument('-bin_conv_rescale',    	dest='bin_conv_rescale', 		 action='store_true',            help='卷积是否可伸缩') 	
	parser.add_argument('-bin_lin_rescale',    	dest='bin_lin_rescale', 		 action='store_true',            help='线性层是否可伸缩') 	
	parser.add_argument('-bin_rescale',    	dest='bin_rescale', 		action='store_true', help='Learnable rescaling in the H dimension (# points)')
